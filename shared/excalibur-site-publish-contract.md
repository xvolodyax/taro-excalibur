...
- **Первый quality 409 ≠ PIPELINE FAIL.** Скрипт пишет
  ...
  - практика уже в теле (B27 / **B29**) → `false_example_409_no_body_edit`.
    После **GATE PASS + H2 практики** не слать Sol на ярлык
    «конкретный пример». Не вставлять «Возьмём:» / «например» / «кейс».
    ...
    разбора ситуации» — ложный (повтор INC-0650 / INC-2035). Не помечать
    «починили сайт». Publish и Директор тело **не** правят.
