...
- **Первый quality 409 ≠ PIPELINE FAIL.** Скрипт пишет
  ...
  - нет H2 практика/чеклист/сценарий/шаг/проверк → `return_sol_practice`
    (Sol пишет практику из фактов этой статьи, затем новый POST, не
    `--resume-article-id`);
  - практика уже в теле (B27 / B29 / B30 / **B32** «Как изменить сценарий…»)
    → `false_example_409_no_body_edit`.
    Детектор `article_has_practice_h2` считает практикой не только
    «практик»/«чеклист», но и H2 со «сценарий» / «проверк» / «шаг».
    После **GATE PASS + H2 практики** не слать Sol на ярлык
    «конкретный пример». Не вставлять «Возьмём:» / «Сцена» /
    «например» / «кейс» / ярлык «конкретный пример».
    В репо **нет** гейта «конкретный пример» / «возьмём / например / кейс».
    Hall: сайт текст **не** бракует. Чекер качества живёт **вне репо**.
    409 при `quality_score=88` и warning «Нет конкретного примера или
    разбора ситуации» — ложный (повтор INC-0650 / INC-2035 / INC-0700).
    SITE token GET quality / force-approve → **403**; это не обход 409
    и не повод править тело. Не помечать «починили сайт».
    Publish и Директор тело **не** правят.
