...
- **Первый quality 409 ≠ PIPELINE FAIL.** Скрипт пишет
  `verdict=needs_sol`, `director_next=return_sol_practice`, exit 2.
  Директор возвращает **Sol**: H2 «Практика: чеклист шагов…» из фактов
  этой статьи + при необходимости фраза про «этот конкретный пример»
  в лиде. Новый POST upload (не `--resume-article-id`: resume не
  заливает тело). Publish **не** правит `article.html` и **не**
  добавляет «Возьмём:». После Sol с практикой пайплайн **продолжается**.
