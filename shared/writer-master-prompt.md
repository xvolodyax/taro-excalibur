...
- **Лид = живая сцена из research этой статьи** (день / место / реплика).
  В конце лида опереться на неё («Разберём этот конкретный пример…») —
  не ярлык «Возьмём:» / «например» / «кейс». Это не гейт сайта и не
  обязательный токен. Не ждать 409.
- **Практика (утро и вечер):** заранее H2 «Практика: чеклист шагов…» из
  маркеров `research-notes.md` этой статьи. Не ждать site 409. Не
  «Возьмём:». Не копировать B23 (часы + H2 «Разбор ситуации» + по минутам).
  Практика/чеклист ≠ ярлык «конкретный пример: ЧЧ:ММ».
