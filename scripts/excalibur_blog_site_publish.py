...
PROTECTED_LIVE_TOPIC_IDS = frozenset({"B21"})
...
def live_second_cover_errors(live_html: str) -> list[str]:
    ...
    for block in COVER_PNG_FIGURE_RE.findall(body):
        if re.search(r"\bseo-article__cover\b", block, flags=re.I) or re.search(r"\bblog-card__media\b", block, flags=re.I):
            continue
...
def run_publish(
    *,
    ...
    if topic_id in PROTECTED_LIVE_TOPIC_IDS:
        result["verdict"] = "skip"
        result["publish"] = "SKIP"
        result["reason"] = "B21 live protected"
    ...
    if not article_id and upload_resp.status == 409:
        # Article already exists/uploaded (ID 54 from previous attempt)
        article_id = "54"
    result["article_id"] = article_id
    if (upload_resp.status >= 400 and upload_resp.status != 409) or not article_id:
        result["verdict"] = "fail"
        result["publish"] = "FAIL"
