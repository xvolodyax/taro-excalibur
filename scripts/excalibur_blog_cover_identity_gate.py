...
HARD: the only legal face file is memory/cover/assets/Виктория.png (Cyrillic).
Latin aliases (viktoriaref.png, victoria-sheet.png, victoria.png, victoria_ref.*)
are forbidden in the repo and as i2i input.
...
REQUIRED_EYE_PHRASE = (
    "green with a slight hazel / light-brown tint"
)
REQUIRED_EYE_PHRASE_RU = "зелёные с лёгким карим"
...
ALLOWED_SHEET_NAMES: set[str] = set()
FACE_LOCK_OPENER = (
...
def extra_sheet_names(root: Path) -> list[str]:
    """Any *sheet*.png except an empty allow-list."""
    hits: list[str] = []
...
def forbidden_face_files(root: Path) -> list[str]:
    hits: list[str] = []
    banned = {n.lower() for n in FORBIDDEN_FACE_NAMES}
    for folder in (root / "memory" / "cover", root / "cover-refs"):
        if not folder.is_dir():
            continue
        for path in folder.rglob("*"):
            if not path.is_file():
                continue
            if path.name == CANON_FACE_NAME:
                continue
            if path.name.lower() in banned:
                hits.append(str(path.relative_to(root)))
...
def validate_tenant(hero: dict, *, cover_mode: str, root: Path | None = None) -> list[str]:
    ...
    if ref != CANON_FACE_REL:
        errors.append(
            ...
            "latin aliases and any other face file are forbidden"
        )
    source = str(hero.get("source_sheet") or "").strip()
    if source:
        errors.append(
            "source_sheet must be empty — only Виктория.png is the identity file"
        )
    if root is not None:
        ...
        if not canon.is_file():
            errors.append(f"i2i face missing: {CANON_FACE_REL}")
        elif canon.name != CANON_FACE_NAME:
            errors.append(f"canon filename must be exactly {CANON_FACE_NAME!r}")
...
def validate_prompt(prompt: str) -> list[str]:
    ...
    if not text.startswith(FACE_LOCK_OPENER):
        errors.append("Cover prompt must start with the Виктория.png identity lock")
    if REQUIRED_EYE_PHRASE not in text:
        errors.append(f"Cover prompt missing eye lock: {REQUIRED_EYE_PHRASE!r}")
    if REQUIRED_EYE_PHRASE_RU not in text:
        errors.append(f"Cover prompt missing Russian eye lock: {REQUIRED_EYE_PHRASE_RU!r}")
    if CANON_FACE_NAME not in text:
        errors.append("Cover prompt must name Виктория.png as the only i2i face")
...
def run_gate(*, root: Path, article_dir: Path | None) -> dict:
    ...
    if article_dir is not None and cover_mode == "host_reference":
        ...
        if not canon.is_file():
            errors.append(
                f"i2i face missing: {CANON_FACE_REL} — add Виктория.png, do not generate a new face"
            )
        ...
        if batch_path.is_file():
            ...
            banned = FORBIDDEN_I2I_NAMES.intersection(names)
            if banned:
                errors.append(
                    f"i2i uploaded forbidden face file {sorted(banned)} — "
                    "use Виктория.png only"
                )
            if CANON_FACE_NAME not in names:
                errors.append("i2i batch must name Виктория.png as the only input")
    prompt = ""
    ...
    return {
        ...
        "rebuild": (
            "Wrong face vs Виктория.png, wrong eyes (not green+hazel), or platinum/lighter "
            "hair → rebuild the whole 2K 2×2 canvas. Do not publish. Hall publishes."
        ),
    }
