...
def is_host_reference(style: dict, hero: dict) -> bool:
    """Victoria / tenant host i2i — not situational cat hero."""
    mode = str(
...
def build_prompt(
    ...
    if host_ref:
        # Compact host_reference lock (B20/B21): identity opener first,
        # gold highlight, no cat-era pink / bold-condensed essay.
        highlight_rule = (
...
def main() -> int:
    ...
    host_ref = is_host_reference(style, hero) and not cat_hero
    if (cat_hero or host_ref or style.get("prefer_local_reference") is True) and local_reference:
        ...
        # host_reference: prefer_local viktoriaref — do not require PUBLIC_SITE_URL.
        batch_ref_url = (
