...
- реф: приложенный `victoria.png` (не переименовывать в `victoria-sheet.png`).
...
- если лицо не Вики / шов — пересобрать холст, не чинить лицо снаружи.
...
- два блока ссылок: после сцены «сразу к делу» (выжимка + 2–3 вопроса к картам + две двери) и свой продающий финал. Не копипаст.
- Дзен: без Telegram.
