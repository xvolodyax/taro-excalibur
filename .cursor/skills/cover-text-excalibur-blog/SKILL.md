...
- `memory/cover/cover-design-code.json` (длина хука, цвет highlight, запрет «bold condensed»)
...
2. `hook` — 2–6 слов у тенанта ТАРО СЕЙЧАС (иначе 2–8): кто + что случилось + зачем мне. Не копируй H1
   дословно, не пиши определение, не метафору без темы.
3. `highlight` — одно слово ИЗ hook. Цвет/насыщенность — `accent_primary` из
   design-code (ТАРО: золото медальона). Не «розовым». Не писать в промпт
   «bold condensed Cyrillic».
