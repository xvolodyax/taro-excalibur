...
class SitePublishUnitTest(unittest.TestCase):
    ...
    def test_first_upload_quality_409_needs_sol(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            article_dir = _write_article(Path(tmp))
            http = FakeHttp()
    ...
    def test_docs_and_doctor_wire_site_publish(self) -> None:
        ...
        self.assertIn("GATE PASS + H2 практики", contract)
        self.assertIn("не слать Sol на ярлык", contract)
        self.assertIn("INC-2035", contract)
        self.assertIn("INC-0700", contract)
        self.assertIn("B30", contract)
        self.assertIn("force-approve", contract)
        self.assertIn("вне репо", contract)
        ...
        self.assertIn("GATE PASS + H2 практики уже в теле", director)
        self.assertIn("не слать Sol на ярлык", director)
        self.assertIn("INC-2035", director)
        self.assertIn("INC-0700", director)
        self.assertIn("B30", director)
        self.assertIn("force-approve", director)
        ...
        self.assertIn("GATE PASS + H2 практики уже в теле", publish)
        self.assertIn("не слать Sol на ярлык", publish)
        self.assertIn("INC-0700", publish)
        self.assertIn("B30", publish)
        self.assertIn("force-approve", publish)
        ...
        self.assertIn("GATE PASS + H2 практики уже в теле", agent)
        self.assertIn("не слать Sol на ярлык", agent)
        self.assertIn("B30", agent)
        self.assertIn("force-approve", agent)
