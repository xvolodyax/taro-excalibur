"""Cover identity gate: only Виктория.png, hair from reference, no platinum."""
...
CANON = ROOT / "memory/cover/assets/Виктория.png"
...
class CoverIdentityTest(unittest.TestCase):
    ...
    def test_canon_face_is_cyrillic_only(self) -> None:
        hero = json.loads((ROOT / "memory/cover/blog-hero.json").read_text(encoding="utf-8"))
    ...
    def test_prompt_injects_hair_lock(self) -> None:
        ...
        self.assertTrue(
            prompt.startswith("same woman as Виктория.png"),
            prompt[:160],
        )
        self.assertIn("Виктория.png", prompt)
        self.assertNotIn("viktoriaref.png", prompt)
        self.assertNotIn("victoria-sheet.png", prompt)
        self.assertNotIn("victoria.png", prompt)
